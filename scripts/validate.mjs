...
const generatedSupabaseSeed = await readFile('supabase/seed.sql', 'utf8');
...
const checks = [
  ...
  ['Package declares Supabase seed SQL generation script', packageJson.includes('"supabase:seed:sql"') && packageJson.includes('scripts/generate-supabase-seed.mjs')],
  ['Environment example declares Supabase and Admin variables', ['NEXT_PUBLIC_SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY', 'SUPABASE_SERVICE_ROLE_KEY', 'BREWMAP_ADMIN_PASSWORD'].every((variable) => envExample.includes(variable))],
  ...
  ['Public page preserves search/map/saved/report DOM contract', ['data-search-form', 'data-filter-row', 'data-cafe-grid', 'data-map-surface', 'data-saved-list', 'data-saved-scope', 'data-saved-auth-state', 'data-report-form', 'data-detail-dialog'].every((selector) => publicPage.includes(selector))],
  ['Public page exposes home content slots', ['data-home-hero-headline', 'data-home-hero-body', 'data-home-notice', 'data-home-curation-card'].every((selector) => publicPage.includes(selector))],
  ...
  ['Public home uses content API with static fallback', js.includes("fetch('/api/content?slug=home'") && js.includes("payload.source !== 'supabase'") && js.includes('applyPublicHomeContent') && js.includes('Using static home content fallback')],
  ...
  ['Main runtime fetches Supabase cafe API with CSV fallback', js.includes("fetch('/api/cafes'") && js.includes("fetch('/data/seed-cafes.csv'") && js.includes('async function loadCafes()') && js.includes("register('/service-worker.js'")],
  ['Main runtime restores saved auth sessions with refresh tokens', js.includes('authSessionExpired') && js.includes('refreshAuthSession') && js.includes('/auth/v1/token?grant_type=refresh_token') && js.includes('await currentAccessToken()')],
  ['Main runtime distinguishes saved auth UI states', ['checking', 'syncing', 'link_sent', 'authenticated', 'offline', 'expired'].every((state) => js.includes(`'${state}'`)) && js.includes('savedScopeText') && js.includes('savedAuthStateText')],
  ['Main runtime rejects HTML deployment-protection data responses', js.includes('assertExpectedResponse') && js.includes('returned an HTML page') && js.includes('Vercel Deployment Protection') && js.includes('Cafe API response is not valid JSON')],
  ...
  ['Service worker skips Vercel login HTML when caching runtime assets', serviceWorker.includes('isVercelLoginResponse') && serviceWorker.includes('Login [–-] Vercel') && serviceWorker.includes('precacheAppShell') && serviceWorker.includes('shouldCacheResponse')],
  ['Service worker excludes Admin and API requests', serviceWorker.includes("requestUrl.pathname === '/admin'") && serviceWorker.includes("requestUrl.pathname.startsWith('/api/')")],
