...
const checks = [
  ...
  ['Retro desktop hides legacy workspace while main shell is active', retroDesktopJs.includes('legacyMount.hidden = true') && retroDesktopJs.includes("legacyMount.setAttribute('aria-hidden', 'true')") && retroDesktopJs.includes('element.hidden = true')],
  ['Retro desktop top menu opens retro programs without legacy scrolling', ['local-zine', 'cafe-index', 'brewmap-map', 'nearby-map', 'brew-log'].every((programId) => retroDesktopJs.includes(`data-open-program="${programId}"`)) && !retroDesktopJs.includes('data-retro-scroll-target')],
