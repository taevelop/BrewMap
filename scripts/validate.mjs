...
const checks = [
  ...
  ['Retro desktop routes top menu actions to retro programs', ['local-zine', 'cafe-index', 'nearby-map', 'brewmap-map', 'brew-log'].every((program) => retroDesktopJs.includes(`data-open-program="${program}"`))],
  ['Retro desktop separates cafe sketch map and nearby map', retroDesktopJs.includes('CAFE SKETCH') && retroDesktopJs.includes('NEARBY MAP') && retroDesktopJs.includes('data-retro-map-locate')],
  ['Retro desktop hides legacy workspace while retro shell is active', retroDesktopJs.includes('legacyMount.hidden = true') && retroDesktopJs.includes("element.hidden = true")],
