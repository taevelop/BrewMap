...
4. 레트로 테마는 우선 **독립 라우트 또는 feature flag** 아래에서 구현한다. 기존 서비스의 기본 화면을 즉시 덮어쓰지 않는다.
...
3. `지도에서 열기`를 누른다.
4. `BREWMAP.EXE`가 해당 카페 위치로 이동한다.
...
│                                                              │
├──────────────────────────────────────────────────────────────┤
│ [BREWMAP] [LOCAL ZINE] [CAFE INDEX] [BREW LOG]     16:33   │  Taskbar
└──────────────────────────────────────────────────────────────┘
```
...
## 8. 공통 지도 프로그램 — `BREWMAP.EXE`

### 8.1 목적

카페의 위치, 주변 관계, 현재 선택 상태를 정확히 보여주는 시스템 프로그램이다.
...
**완료 기준:** 빈 프로그램 창 4개를 실제로 열고 이동·최소화·복원할 수 있다.
...
### Phase 3 — `CAFE_INDEX.EXE` + `BREWMAP.EXE`
...
- [ ] 실험용 라우트에서 기존 사이트와 독립적으로 실행된다.
- [ ] 네 프로그램을 바탕화면 또는 작업 표시줄에서 열 수 있다.
...
- [ ] 기존 BrewMap의 지도, 검색, 저장 플로우를 깨뜨리지 않는다.
- [ ] feature flag를 끄면 기존 화면이 그대로 동작한다.
