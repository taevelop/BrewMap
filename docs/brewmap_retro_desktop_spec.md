...
> 현재 운영 방식: **`/retro`(구 `retro.html`) 별도 진입점의 별관 경험. 공개 홈은 검색 중심 앱이다(2026-06-25 결정, `docs/public-admin-completion-plan.md`)**  
> 문서 상태: 레트로 별관 경험 명세. 기본 진입 화면 기준이 아니며, 본문 중 "하이브리드 공존" 서술은 2026-06-24 이전 시점 기록이다. 방문 도장·개인 메모 등 개인 기록 기능은 별관 전용 실험으로 본 서비스 스코프에 미등재다(`docs/scope.md`)
