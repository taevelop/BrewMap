...
| 로그인 링크 UI 문구 | Fix Applied: `link_sent` 상태의 로그인 링크 발송 문구가 상단 안내와 하단 상태 영역에 중복 표시되어 하단 상태를 비우도록 수정 |
...
| 로그인 링크 UI 문구 | Deploying | 중복 문구 제거 수정 배포 후 `link_sent` 화면에서 문구가 한 번만 보이는지 확인 |
