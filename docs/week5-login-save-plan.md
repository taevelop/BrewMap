...
| 3 | 이메일 로그인 요청 추가 | Partial QA | Supabase Auth OTP 요청 UI 연결, 이메일 미입력 검증 완료, 실메일 링크 수신 검증 필요 |
| 4 | 로그인 콜백 처리 | Implemented | URL access token 저장 및 `#saved` 정리, 실메일 콜백 검증 필요 |
| 5 | 임시 저장 병합 | Implemented | 로그인 콜백 시 localStorage 저장을 서버 저장 목록에 병합, 실토큰 검증 필요 |
...
| 매직링크 Redirect | Fix Applied: Supabase OTP 요청 URL에 `redirect_to`를 명시. 기존 localhost 링크는 폐기하고 새 링크로 재검증 필요 |
| 비로그인 저장 | Pass: 상세 모달 저장, 저장 카운트 1개, localStorage 저장 확인 |
| 실메일 링크 | Pending: 실제 이메일 주소와 메일함 확인 필요 |
| 실토큰 서버 저장/병합 | Pending: 이메일 링크 콜백 access token 필요 |
