...
진행 상태의 단일 소스는 `docs/work-board.md`다. 이 문서는 Week 5 구현 계획, API/상태 설계, 날짜별 QA 기록을 담는다.
...
작업별 최신 상태는 `docs/work-board.md`의 "다음 목표: 로그인/저장" 표에서만 관리한다. 세부 QA 근거는 아래 날짜별 QA 결과 절을 참조한다.
...
| 항목 | 결과 |
| --- | --- |
| 기준 브랜치 | `codex/week5-qa-closeout` |
| 로컬 빌드 | Pass: `NEXT_PUBLIC_SUPABASE_URL=http://localhost:3000/mock-supabase`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=local-qa-key`로 `npm run build` 통과 |
| headless Chrome QA | Pass: Chrome CDP 자동화로 비로그인 메뉴/작업 공간 분리, pending action, localStorage refresh, refresh token 상태 전환 확인 |
| 비로그인 메뉴 | Pass: 상단 메뉴가 발견/결과/지도/로그인 중심으로 표시되고 저장/제보 메뉴와 작업 공간은 숨김 |
| 비로그인 저장 pending | Pass: 상세 모달 저장 클릭 시 `brewmap.authPendingAction.v1`에 `{ type: "save", cafeId }` 저장 후 `/login` 이동, 모의 세션 주입 후 `/#saved` 복귀, 저장 1개 반영, pending action 제거 |
| 비로그인 제보 pending | Pass: 상세 모달 정보 제보 클릭 시 `{ type: "report", cafeId }` 저장 후 `/login` 이동, 모의 세션 주입 후 `/#report` 복귀와 제보 카페 선택 복원 |
| 비로그인 localStorage 새로고침 | Pass: `brewmap.savedCafes.v1`에 `busan-jung-airy-coffee` 저장 후 새로고침해도 저장 카운트 1개와 저장 목록 DOM 유지. 작업 공간은 비로그인 상태라 계속 hidden |
| refresh token 세션 복구 | Pass: 만료된 세션과 `valid-refresh` 모의 응답에서 `refreshAuthSession` 경로가 새 access token을 저장하고 UI가 `authenticated` / `로그인됨` 상태 유지 |
| invalid refresh token | Pass: `invalid-refresh` 모의 응답 400에서 저장 세션을 제거하고 UI가 `expired` / `다시 로그인 필요` 상태로 전환 |
| production 페이지 | Pass: `https://www.brewmapkr.com/` 200 |
| production 저장 API 무인증 | Pass: `GET /api/saved-cafes` 401, `POST /api/saved-cafes` 401 |
| production DELETE 확인 | Not run: 무인증 `DELETE /api/saved-cafes`는 인증 경계가 깨졌을 때 실제 데이터 삭제 위험이 있어 자동 실행하지 않음. 운영자 명시 승인 후 확인 필요 |

운영자 확인 항목:

- 실메일 매직링크 로그인 후 production 새로고침에서 `로그인됨` / `계정 저장` 유지 확인.
- Google OAuth 실복귀가 루트 redirect 후 pending 저장은 `#saved`, pending 제보는 `#report`로 이어지는지 확인.
- production `DELETE /api/saved-cafes` 무인증 401은 실제 삭제 위험을 이해하고 명시 승인한 상태에서만 확인.
...
| 새로고침 세션 유지 | Done (자동 QA) | 2026-07-07 headless Chrome에서 valid refresh token 복구와 invalid refresh token 만료 전환 확인. 실메일 production 새로고침은 운영자 확인 항목 |
